...
| `scripts/flop_address.py` + `scripts/flop_rpc_probe.py` + `docs/TESTNET-READINESS.md` | FLOP testnet day-one kit for the agent (compute-buyer) side: did:key → SS58 account for any prefix (self-tested), read-only first-contact probe of a node (chain, `ss58Format`, agent pallets in metadata), and a sourced runbook of what the Yellow Paper specifies vs leaves [TBD] | testnet readiness |
| `boardintel.py` + `board.html` + `scripts/build_board.py` | tclk board intelligence: who wins an offer and how fast (first-bidder rule, quartile latencies), which posters actually judge and let newcomers in, task families, two populations of bidders (84% of accept lines are non-conformant), reveal-ref hazard, strict fold and signature checks — all from two public exports, re-measured every 30 min by a recorder; unit-tested | board intelligence (novel) |
| `flopspend/` + `docs/SPEND-PLAN.md` | offline core of the FLOP-testnet spending agent: vendored canonical compute-channel encoders (CC BY 4.0) checked against the published wire-format corpus (42/52 pass, 10 skipped with reasons), agent-side transcript/receipt, pacing budget, spend ledger, workload queue, and a dry-run miner+chain simulator; the real chain client waits for the RPC | testnet spend runner |
